...
2. KKT 点只能求到可能的正则局部极小点（是否真的为局部极小点，则要通过二阶条件验证，或是凸规划），不能求出不正则的局部极小点。  
   故对于**凸规划问题**，只有满足**所有可行点均是正则点**时，才能直接**通过求 KKT 点的方法**求出最优解。
...
>    $$
>
> 则称：$\boldsymbol{x}_0$具有 / 满足 MFCQ（或说$\boldsymbol{x}_*$处 MFCQ 成立）。
...
> 3. 存在**可行点**$\bar{\boldsymbol{x}}$，使得**积极约束为非退化**的：  
>    $$
...
一般来说：只要规划问题**提前满足条件 1,2**，对于某点的积极约束，其**相交区域不为一个点**，那么就满足 Slater 条件。  
所以该条件很好用。

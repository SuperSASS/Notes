...
* **（凸）锥** - 集合  
  * 几何上：集合$C$一点$\boldsymbol{x}$，以原点为端点作射线也在$C$中（$\mathbb{R}^2$里$C$为一/两个**过原点**射线形成的线/面区域；$\mathbb{R}^3$中为若干条**过原点**射线形成的线/面/体）  
  定义上：$\forall\boldsymbol{x}\in C, \alpha\ge0$，$C$为**凸**的且$\alpha\boldsymbol{x}\in C$
* 锥组合 - 操作  
  $$
  \sum_{i=1}^k \alpha_i\boldsymbol{x_i}\quad(\alpha_i\ge0)
  $$
  即系数**非负的线性组合**。
...
* **对称矩阵锥** - $S^n=\{A\in\mathbb{R}^{n*n}:A=A^T\}$  
  *注：这个和以下两个锥的集合元素为矩阵，不能再用几何去理解，只是说明这个几何中所有矩阵具有锥的性质。*  

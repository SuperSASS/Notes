...
除了指定使用某个命名空间(`using namespace ...`)，  
还可以只用某个成员的声明。

如：`using std::cout`，  
则以后直接使用`cout`就行，但其他的(比如`endl`)都要加`std::`。

## 二、字符串

* `s = cin.get();` - 只接收一个字符，等同于`getchar`。
* `cin.get(s,n)` - 对字符串s读取长度为n的字符串。
* `cin.getline(s,n,ch)` - 接受长度最大为n的字符串（包括空格），结束字符为ch（遇到提前结束）
...
* 引用并**没有开辟新的空间**，与被引用变量都是同一个变量，共用空间。
* 引用后，该变量不能再引用其他变量。
...
int *p2 = 0x12345678; //随便一串地址
*p2 = r; //这里是：把r赋值给p2指向的变量，不是把p2指向r！
```

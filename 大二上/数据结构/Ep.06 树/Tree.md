...
1. 对于给定的$n$个数$\{w_1,w_2,\cdots,w_n\}$，先分别将他们作为$n$个树，并且根节点的值即是结点权值。
2. 将**当前根节点的值最小**的两棵树合并。  
   合并时不要求左右子树顺序。
3. 合并后的树，其**根节点的值为左右两棵子树的值的和**。（而不是计算带权路径长度，是**直接求和**！）
4. 重复$n-1$次步骤2,直到形成一棵树。该树即为哈夫曼树。
...
> 证明这种构造方式的正确性：
>
> 可以先证明：该树的$WPL$值等于所有非叶子结点上值的和。
> 因为非叶子结点上的和为左右子树相加，假如先合并两叶子结点，左叶子$w_l=A$，右叶子$w_r=B$，这其父节点$w_p=A+B$，此时计算$WPL=w_l+w_r=A+B=w_p$，  
> 接下来再将此树跟另一个叶子$w_3=C$合并，则父节点$w_p'=w_p+w_3=A+B+C$，计算$WPL=2w_l+2w_r+w_3=2A+2B+C=w_p+w_p'$，  
> 如此迭代上去，在**每一个非叶子节点，都会加上其叶子节点的权值**，就相当于让叶子节点路径长度加一，  
> 故可证明上结论。
>
> 然后转换成非叶子节点值的和后，那么构造出最小的非叶子节点值的和，  
> 简单的利用贪心，每次选择最小两个出来构建，一定能保证最终的和最小。
>
> 比如对于$1,2,3,4,5,6,7$，应该选$1,2\rightarrow3$，  
> 构建好后还要将$3$放回去，变为$3,3,4,5,6,7$继续构建。  
> 假如要选择别的大一点的，如$1,3$构建，那么**不仅这里**构建出的结点**值为$4$要大一些**，  
> 放回去后变为$4,3,4,5,6,7$**让之后构建时会更大**。  
> 所以肯定要选最小的两个构建。
...
void buildHFT(typeHFT &t)
{
  ...
  for (int i=t.n; i<m; i++) //开始合并，n-1次
  {
    for (int min=0; min<i; min++) //之前可能存在非根结点，用来跳过
      if (t.hf[min].parent==-1)
        break;
    for (int find=min; find<i; find++) //寻找最小权重结点，范围到目前的有效结点i

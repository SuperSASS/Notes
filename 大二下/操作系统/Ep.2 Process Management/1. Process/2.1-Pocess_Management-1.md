...
进程的两个基本元素时**程序代码**(Program code)和与代码相关联的**数据集**(Set of data)。
...
### 2. The creation and Termination of Process - 进程的创建和终止
...
一般进程都运行在用户模式，  
当需要调用内核功能时，则要切换到内核模式，称为模式切换(Mode Switch)。  
